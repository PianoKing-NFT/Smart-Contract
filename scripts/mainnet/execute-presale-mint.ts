...
async function main() {
  ...
  const pianoKingWhitelist = await ethers.getContractAt(
    "PianoKingWhitelist",
    "0xB2E31C3D51bbfefB4653789CF0965f9dfa7C902a"
  );

  const addresses = await pianoKingWhitelist.getWhitelistedAddresses();
  ...
  console.log("Presale mint completed successfully!");
}
